...
public sealed class DatabaseEmptier
{
    ...
    public static void DeleteAllData(IConfiguration config)
    {
        if (_deleteSql == null)
        {
            _deleteSql = BuildDeleteTableSqlStatement(config);
        }
    ...
    private static string BuildDeleteTableSqlStatement(IConfiguration config)
    {
        IList<string> allTables = GetAllTables(config);
        IList<Relationship> allRelationships = GetRelationships(config);
    ...
    private static IList<Relationship> GetRelationships(IConfiguration config)
    {
        var relationships = new List<Relationship>();
        SqlExecuter.ExecuteSql(config, 
            @"select
...
,   so_fk.name;",
    ...
    private static IList<string> GetAllTables(IConfiguration config)
    {
